...
def run_softmax_on_MNIST(temp_parameter=1):
    ...
    #test_error_mod3 = compute_test_error_mod3(test_x, updated_y[1], theta, temp_parameter)
    #print('softmax mod3 test_error=', test_error_mod3)
    return test_error
...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    theta, cost_function_history = softmax_regression(train_x, updated_train_y, temp_parameter, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
    plot_cost_function_over_time(cost_function_history)
    test_error = compute_test_error(test_x, updated_test_y, theta, temp_parameter)
    # Save the model parameters theta obtained from calling softmax_regression to disk.
    write_pickle_data(theta, "./theta.pkl.gz")
...
theta, cost_function_progression = softmax_regression(train_pca, train_y, temp_parameter=1, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
test_error = compute_test_error(test_pca, test_y, theta, temp_parameter=1)
print("18PCA Test Error:", test_error)
...
svc_poly = SVC(random_state=0, kernel='poly', degree=3)
svc_poly.fit(train_pca10, train_y)
predictions = svc_poly.predict(test_pca10)
test_error = np.mean(predictions != test_y)
print("Test error poly svc:",test_error)

svc_rbf = SVC(random_state=0, kernel='rbf', degree=3)
svc_rbf.fit(train_pca10, train_y)
predictions = svc_rbf.predict(test_pca10)
test_error = np.mean(predictions != test_y)
print("Test error rbf svc:",test_error)
...
theta, cost_function_progression = softmax_regression(train_cube, train_y, temp_parameter=1, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
test_error = compute_test_error(test_cube, test_y, theta, temp_parameter=1)
print("10PCA Test Error:", test_error)
